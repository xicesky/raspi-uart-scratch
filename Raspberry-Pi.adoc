...
* Raspberry Pi documentation: https://www.raspberrypi.com/documentation/computers/configuration.html#configure-uarts
...
To enable the primary UART (PL011 on my RPi2) it is enough to configure it via raspi-config as described in https://www.raspberrypi.com/documentation/computers/configuration.html#disabling-the-linux-serial-console[the raspberry documentation].

UART signal:
image:doc/RS-232_timing.svg[]

This image from wikipedia shows the logical signal, the UART on RPi's GPIO ports seems to follow the rule that GND is a logical zero and +3.3V is a logical one.
...
Many old tutorial point to the old raspberry toolchain (https://github.com/raspberrypi/tools), which is deprecated.
Ubuntu has support for armhf, which can simply be installed via `sudo apt-get install gcc-arm-linux-gnueabihf`.

Some hints for cross-compiling with https://manpages.ubuntu.com/manpages/jammy/man1/mk-sbuild.1.html[mk-sbuild] (see also https://wiki.debian.org/sbuild[sbuild]) can be found https://earthly.dev/blog/cross-compiling-raspberry-pi/[here], but many parts of this article are out-of-date (especially the note "The cross-compilation toolchains in the Ubuntu repository are not compatible with Raspberry Pi boards").

=== Rust

Make sure you have docker (or podman) installed -- this is required for cross.
Install rust as per usual (https://rustup.rs/), install https://github.com/cross-rs/cross?tab=readme-ov-file[cross] via cargo: `cargo install cross`

* https://github.com/rpi-pal/rpi-pal (the currently maintained (?) version of https://github.com/golemparts/rppal[rppal] according to https://www.reddit.com/r/rust/comments/1o59voa/rppal_forking_and_maintaining/[this reddit thread])

=== Scratch

https://www.raspberrypi.org/blog/scratch-3-desktop-for-raspbian-on-raspberry-pi/

In order to write scratch from proper text files instead of having to use drag and drop:

* https://scratch.mit.edu/discuss/topic/792714/?page=1#post-8263820
* https://github.com/ScrybeLang/ScratchGen/tree/main/src/ScratchGen
* https://github.com/DavidBuchanan314/boiga

=== Python

Probably won't use python, but if i still do, i'd like to use https://textual.textualize.io/guide/workers/[textual workers].
